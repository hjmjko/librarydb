...
--4.1.5
DELIMITER //
CREATE PROCEDURE find_operators_with_same_loan_count()
BEGIN
...
BEGIN
  ...
  -- Get the maximum number of books written by any author
  ...
  -- Set the threshold value as five books less than the maximum
  SET threshold = max_books - 5;

-- Find the author with the most books and their count
  ...
  -- Find authors who have written at least five books less than the maximum
  SELECT A.author_id, A.first_name, A.last_name, COUNT(*) AS num_books
  FROM Author A
  JOIN Book_Author BA ON A.author_id = BA.author_id
  GROUP BY A.author_id, A.first_name, A.last_name
...
--4.2.3
DROP PROCEDURE IF EXISTS FindAverageRatings;
DELIMITER //
...
CALL FindAverageRatings(NULL, NULL, NULL);

CALL FindAverageRatings(1, 'Science Fiction', 123);
...
CREATE PROCEDURE UserSearchBorrowedBooks(
  IN userIdCriteria INT UNSIGNED
)
BEGIN
  SELECT DISTINCT bk.ISBN, bk.title, GROUP_CONCAT(DISTINCT CONCAT(a.first_name, ' ', a.last_name) SEPARATOR ', ') AS authors,
         (
           SELECT GROUP_CONCAT(DISTINCT c.category_name SEPARATOR ', ')
           FROM Category AS c
           INNER JOIN Book_Category AS bc ON c.category_id = bc.category_id
           WHERE bc.ISBN = bk.ISBN
         ) AS categories
  FROM Book AS bk
  INNER JOIN Book_Author AS ba ON bk.ISBN = ba.ISBN
  INNER JOIN Author AS a ON ba.author_id = a.author_id
  INNER JOIN School_Book AS sb ON bk.ISBN = sb.ISBN
  INNER JOIN Book_Category AS bc ON bk.ISBN = bc.ISBN
  INNER JOIN Category AS c ON bc.category_id = c.category_id
  INNER JOIN Borrowing AS br ON bk.ISBN = br.ISBN
  WHERE br.user_id = userIdCriteria
  GROUP BY bk.ISBN;
END //
